...
# This program will ask to enter the valid operation number,
# if entered number is wrong
...
while prompt.to_i < 1 || prompt.to_i > 5
  puts 'Wrong number, please enter the valid operation number'
  prompt = gets.chomp
end
...
case prompt.to_i
  ...
  when 4 then
    ...
    puts "#{first_number} modulo #{second_number} is: #{modulo(first_number, second_number)}"
end